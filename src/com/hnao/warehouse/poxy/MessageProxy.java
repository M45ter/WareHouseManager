...
public class MessageProxy extends BaseProxy {
	...
	public BizResult<Integer> GetNotRead(String opName) {
		...
		try {
			...
			params.addQueryStringParameter("opName", opName);

			ResponseStream responseStream = http.sendSync(HttpMethod.GET, ComConst.SERVICE_URL + "Message/GetNotRead",
					params);
			...
			if (ValidateResult(strResponse)) {
				re = new BizResult<Integer>();
				Type type = new TypeToken<BizResult<Integer>>() {
				}.getType();
	...
	public BizResult<List<Message>> GetAllMessages(ListMessage listmessage) {
		...
		try {
			...
			Gson gson = gsonb.create();
			String strPost = gson.toJson(listmessage);
			...
			ResponseStream responseStream = http.sendSync(HttpMethod.POST,
					ComConst.SERVICE_URL + "Message/GetAllMessages", params);
			String strResponse = responseStream.readString();
			Log.d("", "strResponse = " + strResponse);
			if (ValidateResult(strResponse)) {
				re = new BizResult<List<Message>>();
				Type type = new TypeToken<BizResult<List<Message>>>() {
				}.getType();
				re = gson.fromJson(strResponse, type);
			}
		} catch (Exception e) {
	...
	public BizResult<Message> EditMessages(Message messages) {
		...
		try {
			...
			gsonb.setDateFormat("yyyy-MM-dd HH:mm:ss.SSS");
			Gson gson = gsonb.create();
			String strPost = gson.toJson(messages);
			Log.d("", "strPost = " + strPost);
			RequestParams params = new RequestParams();
			if (configCheckCompany) {
				SharedPreferences sp = _context.getSharedPreferences(ComConst.SP_LOGIN, Context.MODE_PRIVATE);
			...
			ResponseStream responseStream = http.sendSync(HttpMethod.POST,
					ComConst.SERVICE_URL + "Message/EditMessages", params);
			String strResponse = responseStream.readString();
			Log.d("", "strResponse = " + strResponse);
			if (ValidateResult(strResponse)) {
				re = new BizResult<Message>();
				Type type = new TypeToken<BizResult<Message>>() {
				}.getType();
				re = gson.fromJson(strResponse, type);
			}
		} catch (Exception e) {
