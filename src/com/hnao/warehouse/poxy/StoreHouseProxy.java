...
public class StoreHouseProxy extends BaseProxy {

	private static StoreHouseProxy _instance;
	
	private static boolean configCheckCompany;

	public static StoreHouseProxy getInstance(Context context) {
		if (_instance == null) {
			_instance = new StoreHouseProxy();
		}
		_instance._context = context;
		return _instance;
	}

	public BizResult<List<StoreHouse>> GetStoreHouseList(StoreHouseArgs houseArgs) {
		...
		try {
			...
			String strPost = gson.toJson(houseArgs);
			...
			ResponseStream responseStream = http.sendSync(HttpMethod.POST,
					ComConst.SERVICE_URL + "StoreHouse/GetStoreHouseList", params);
			String strResponse = responseStream.readString();
			Log.d("", "strResponse = " + strResponse);
			if (ValidateResult(strResponse)) {
				re = new BizResult<List<StoreHouse>>();
				Type type = new TypeToken<BizResult<List<StoreHouse>>>() {
				}.getType();
