...
public class EditApplyStatusTask extends BaseAsyncTask<Apply, String, BizResult<Apply>> {

	public final static String TAG = "EditApplyStatusTask";

	private TaskCompletedListener mListener;

	public EditApplyStatusTask(Context context) {
		super(context);
		// TODO Auto-generated constructor stub
	}

	public EditApplyStatusTask(Context context, boolean showDialog) {
	...
	protected BizResult<Apply> doInBackground(Apply... params) {
		...
		if (null != params && params.length > 0) {
			try {
				Re = ApplyProxy.getInstance(mContext).EditApplyStatus(params[0]);
			} catch (Exception e) {
	...
	protected void onPostExecute(BizResult<Apply> result) {
		...
		if (result != null && result.Success) {
			Log.d(TAG, "EditApplyStatus success!");
			if (mListener != null) {
				mListener.resultSucceed(result);
			}
		} else {
			Log.d(TAG, "EditApplyStatus failed!");
			if (mListener != null) {
